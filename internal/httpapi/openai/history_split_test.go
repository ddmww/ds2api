...
func TestChatCompletionsHistorySplitCanInlineHistoryWhenFileModeDisabled(t *testing.T) {
	useFile := false
	ds := &inlineUploadDSStub{}
	h := &openAITestSurface{
		Store: mockOpenAIConfig{
	...
	if len(ds.uploadCalls) != 0 {
		t.Fatalf("expected no upload call when use_file=false, got %d", len(ds.uploadCalls))
	}
	if ds.completionReq == nil {
		t.Fatal("expected completion payload to be captured")
	}
	promptText, _ := ds.completionReq["prompt"].(string)
	if !strings.Contains(promptText, "Previous conversation history") ||
		!strings.Contains(promptText, "first user turn") ||
		!strings.Contains(promptText, "latest user turn") {
		t.Fatalf("expected inline history and latest turn in prompt, got %s", promptText)
	}
	if strings.Contains(promptText, "[file content end]") || strings.Contains(promptText, "[file name]: IGNORE") {
		t.Fatalf("expected grok inline history without file wrapper, got %s", promptText)
	}
}
