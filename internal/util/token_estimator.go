...
const (
	tokenProviderOpenAI tokenProvider = "openai"
	tokenProviderGemini tokenProvider = "gemini"
	tokenProviderClaude tokenProvider = "claude"
)
...
var providerMultipliers = map[tokenProvider]tokenMultipliers{
	tokenProviderGemini: {
...
func EstimateTokens(text string) int {
	return EstimateTokensByModel("", text)
}

func EstimateTokensByModel(modelName, text string) int {
	if text == "" {
		return 0
	}
	return estimateTokensForProvider(providerForModel(modelName), text)
}

func EstimateAnyTokens(value any) int {
	return EstimateAnyTokensByModel("", value)
}

func EstimateAnyTokensByModel(modelName string, value any) int {
	data, _ := json.Marshal(value)
	return EstimateTokensByModel(modelName, string(data))
}

func providerForModel(modelName string) tokenProvider {
	name := strings.ToLower(strings.TrimSpace(modelName))
	switch {
	case strings.Contains(name, "gemini"):
		return tokenProviderGemini
	case strings.Contains(name, "claude"):
		return tokenProviderClaude
	default:
		return tokenProviderOpenAI
	}
}

func estimateTokensForProvider(kind tokenProvider, text string) int {
	m, ok := providerMultipliers[kind]
	if !ok {
		m = providerMultipliers[tokenProviderOpenAI]
	}
	var count float64
